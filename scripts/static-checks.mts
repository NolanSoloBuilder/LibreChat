...
/** The config the design rules are configured by: narrowing a rule or widening
 *  a contract there changes what every recorded count stands for. */
const DESIGN_CONFIG = 'eslint.config.mjs';
 ...
 * the diff changes a component-library source or the rules' own config, because
 * a primitive's variants and the config's contracts both decide what the rules
 * report in every caller. A baseline named in the diff is always checked in
 * full: what it silences is its whole purpose.
 */
