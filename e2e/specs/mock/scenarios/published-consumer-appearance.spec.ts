...
`,
    ...
    /** And the step is still a variable, so a theme retunes it: the SPA sets
     *  `--radius: 0.5rem`, which is where its own 4px comes from. */
    await page.evaluate(() => document.documentElement.style.setProperty('--radius', '0.5rem'));
    expect((await read('box', ['border-radius']))['border-radius']).toBe('4px');
