...
test.describe('design-system rules', () => {
  ...
  test('a jsx client file is checked by the design rules @scenario:a-jsx-client-file-is-checked-by-the-design-rules', () => {
     ...
     *  exemption block has to win for the same source under every pattern the
     *  repository's tests really use, including the `.tsx` ones the client
     *  config lints. */
    for (const path of [
      ...
      expect(designMessages(lintStdin(path, INLINE_DISPLAY)), path).toEqual([]);
    }
